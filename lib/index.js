 ...
 * @property {RecipeCollection[]} recipeCollections
...
class AnyList extends EventEmitter {
   ...
   */
	...
	/**
	 * Load undated meal planning events, which AnyList uses for the Meal Plan Queue.
	 * @return {Promise<MealPlanningCalendarEvent[]>} queue events
	 */
	async getMealPlanQueue(refreshCache = true) {
		const events = await this.getMealPlanningCalendarEvents(refreshCache);
		return events.filter(event => event.isQueued);
	}
   ...
   */
	...
	/**
	 * Factory function to create an undated Meal Plan Queue event.
	 * Call save() on the returned event to add it to AnyList.
	 * @param {object} event new queue event options
	 * @return {Promise<MealPlanningCalendarEvent>} queue event
	 */
	async createMealPlanQueueEvent(event = {}) {
		return this.createEvent({...event, date: null, eventType: 1});
	}

	/**
	 * Add a recipe to the Meal Plan Queue and save it to AnyList.
	 * @param {Recipe|string} recipe recipe object or recipe identifier
	 * @param {object} eventOptions optional queue event fields
	 * @return {Promise<MealPlanningCalendarEvent>} saved queue event
	 */
	async addRecipeToMealPlanQueue(recipe, eventOptions = {}) {
		const recipeId = typeof recipe === 'string' ? recipe : recipe?.identifier;
		if (!recipeId) {
			throw new TypeError('A recipe or recipe identifier is required');
		}

		const loadedRecipe = typeof recipe === 'string'
			? this.recipes.find(candidate => candidate.identifier === recipe)
			: recipe;
		const event = await this.createMealPlanQueueEvent({
			...eventOptions,
			recipeId,
			title: eventOptions.title ?? loadedRecipe?.name,
		});
		await event.save();
		return event;
	}
   ...
   */
	...
	/**
	 * Upload a photo for later attachment to a recipe.
	 * AnyList stores the generated photo ID in PBRecipe.photoIds when the recipe is saved.
	 * @param {string} filePath local image path
	 * @param {object} options upload options
	 * @param {string=} options.photoId optional 32-character hexadecimal photo ID
	 * @param {string=} options.contentType optional image MIME type override
	 * @return {Promise<{photoId: string, filename: string}>} uploaded photo details
	 */
	async uploadPhoto(filePath, {photoId = uuid(), contentType} = {}) {
		if (typeof filePath !== 'string' || filePath.trim() === '') {
			throw new TypeError('A local photo path is required');
		}

		if (!/^[a-f\d]{32}$/i.test(photoId)) {
			throw new TypeError('Photo ID must be 32 hexadecimal characters');
		}

		const resolvedPath = path.resolve(filePath);
		const inferredContentType = contentType || PHOTO_CONTENT_TYPES.get(path.extname(resolvedPath).toLowerCase());
		if (!inferredContentType || !inferredContentType.startsWith('image/')) {
			throw new TypeError('Photo must use a supported image extension or an image content type');
		}

		const photo = await fs.promises.readFile(resolvedPath);
		if (photo.length === 0) {
			throw new TypeError('Photo file is empty');
		}
		...
		await this.client.post('data/photos/upload', {
			body: form,
			headers: {
	...
	/**
	 * Upload a local photo, attach its ID to a recipe, and save the recipe.
	 * @param {Recipe|string} recipe recipe object or loaded recipe identifier
	 * @param {string} filePath local image path
	 * @param {object} options upload options passed to uploadPhoto
	 * @return {Promise<{recipe: Recipe, photo: {photoId: string, filename: string}}>} result
	 */
	async addPhotoToRecipe(recipe, filePath, options = {}) {
		const loadedRecipe = typeof recipe === 'string'
			? this.recipes.find(candidate => candidate.identifier === recipe)
			: recipe;
		if (!loadedRecipe || typeof loadedRecipe.save !== 'function') {
			throw new TypeError('A loaded recipe or recipe identifier is required');
		}

		const photo = await this.uploadPhoto(filePath, options);
		const previousPhotoIds = [...(loadedRecipe.photoIds || [])];
		loadedRecipe.photoIds = [...new Set([...previousPhotoIds, photo.photoId])];
		try {
			await loadedRecipe.save();
		} catch (error) {
			loadedRecipe.photoIds = previousPhotoIds;
			throw error;
		}

		return {recipe: loadedRecipe, photo};
	}
   ...
   */
	...
	/**
	 * Get a loaded RecipeCollection instance by ID.
	 * @param {string} identifier collection ID
	 * @return {RecipeCollection|undefined} collection
	 */
	getRecipeCollectionById(identifier) {
		return this.recipeCollections.find(collection => collection.identifier === identifier);
	}

	/**
	 * Get a loaded RecipeCollection instance by name.
	 * @param {string} name collection name
	 * @return {RecipeCollection|undefined} collection
	 */
	getRecipeCollectionByName(name) {
		return this.recipeCollections.find(collection => collection.name === name);
	}
